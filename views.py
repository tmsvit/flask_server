...
def tt_history():
    ...
    if user.group == "superadmin" or user.group == "ttfaculty":
        ...
        for get_tt_history in get_tt_history:
            result.append({"tt_id": get_tt_history.tt_id, "tt_dept": get_tt_history.tt_dept, "tt_name": get_tt_history.tt_name,
                          "tt_unique_id": get_tt_history.tt_unique_id, "tt_status": get_tt_history.tt_status, "tt_date": get_tt_history.tt_date, "tt_time": get_tt_history.tt_time})
        ...
        for get_tt in get_tt:
            hodresult.append({"tt_id": get_tt.tt_id, "tt_dept": get_tt.tt_dept, "tt_name": get_tt.tt_name,
                             "tt_unique_id": get_tt.tt_unique_id, "tt_status": get_tt.tt_status, "tt_date": get_tt.tt_date, "tt_time": get_tt.tt_time})
...
def makebacktonormal(tt_id):
    ...
    if user.group == "superadmin":
        ...
        tt_data.tt_public_key = generate_password_hash(
            str(os.getenv('TT_PUBLIC_KEY')))
        db.session.commit()
...
def check_public(tt_id):
    ...
    if validate_publicity():
        if tt_data.tt_is_public == "0" and check_password_hash(tt_data.tt_public_key, os.getenv('TT_PUBLIC_KEY')) and user.group == "ttfaculty":
            new_hash_status = generate_password_hash(
                str(os.getenv('FINAL_PUBLIC_KEY')))
...
def enrollment():
    ...
    dbC.execute(
        "selct * from ttgen_course where course_number = %s", (course_number))

    cdata = dbC.fetchone()

    total_cnt = Enroll.query.filter_by(course_number=course_number).count()

    if (total_cnt >= cdata.max_numb_student):
        return {"message": "Course is capacity is full"}

    enrl = Enroll(student_id=student_id, section=section, course_number=course_number, course_name=course_name,
                  room=room, instructor_id=instructor_id, instructor_name=instructor_name, pid=pid, time=time, day=day)
    db.session.add(enrl)
    db.session.commit()
